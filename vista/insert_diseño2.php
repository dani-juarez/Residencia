...
<?php 
	...
	if(isset($_POST['guardar'])){
		...
		if(!empty($edad) && !empty($hombres) && !empty($mujeres) && !empty($hombres_dis) && !empty($mujeres_dis) && !empty($hombres_total) && !empty($mujeres_total) && !empty($total) ){
				...
				$consulta_insert=$con->prepare('INSERT INTO diseño2(edad,hombres,mujeres,hombres_dis,mujeres_dis,hombres_total,mujeres_total,total) VALUES(:edad,:hombres,:mujeres,:hombres_dis,:mujeres_dis,:hombres_total,:mujeres_total,:total)');
...
<body>
	<div class="contenedor">
		<h2>INGENIERIA EN DISEÑO INDUSTRIAL</h2>
