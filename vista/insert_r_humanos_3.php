...
<?php 
	...
	if(isset($_POST['guardar'])){
		$prof_hora_tiempo=$_POST['prof_hora_tiempo'];
		...
		if(!empty($prof_hora_tiempo) && !empty($licenciatura) && !empty($especialidad) && !empty($maestria) && !empty($maestria_s_a) && !empty($doctorado) && !empty($doctorado_s_a) ){
			
				$consulta_insert=$con->prepare('INSERT INTO r_humanos_3(prof_hora_tiempo,licenciatura,especialidad,maestria,maestria_s_a,doctorado,doctorado_s_a) VALUES(:prof_hora_tiempo,:licenciatura,:especialidad,:maestria,:maestria_s_a,:doctorado,:doctorado_s_a)');
				$consulta_insert->execute(array(
                    ':prof_hora_tiempo' =>$prof_hora_tiempo,
...
<body>
	<div class="contenedor">
		<h2>NUEVO PROFESOR DE HORAS ASIGNADAS</h2>
		<form action="" method="post">
			<div class="form-group">
				<input type="text" name="prof_hora_tiempo" placeholder="PROFESOR DE HORAS ASIGNADAS" class="input__text">
