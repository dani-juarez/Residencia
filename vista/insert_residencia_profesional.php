...
<?php 
	...
	if(isset($_POST['guardar'])){
		...
		if(!empty($programa) && !empty($modalidad) && !empty($hombres_solicitantes) && !empty($mujeres_solicitantes) && !empty($hombres_aceptados) && !empty($mujeres_aceptados) ){
				...
				$consulta_insert=$con->prepare('INSERT INTO residencia_profesional(programa,modalidad,hombres_solicitantes,mujeres_solicitantes,hombres_aceptados,mujeres_aceptados) VALUES(:programa,:modalidad,:hombres_solicitantes,:mujeres_solicitantes,:hombres_aceptados,:mujeres_aceptados)');
