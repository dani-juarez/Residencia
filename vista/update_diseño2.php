...
<?php
	...
	if(isset($_GET['id'])){
		...
		$buscar_id=$con->prepare('SELECT * FROM diseño2 WHERE id=:id LIMIT 1');
	...
	if(isset($_POST['guardar'])){
		...
		if(!empty($edad) && !empty($hombres) && !empty($mujeres) && !empty($hombres_dis) && !empty($mujeres_dis) && !empty($hombres_total) && !empty($mujeres_total) && !empty($total) ){
				...
				$consulta_update=$con->prepare(' UPDATE diseño2 SET  
					edad=:edad,
					hombres=:hombres,
					mujeres=:mujeres,
...
<body>
	<div class="contenedor">
		<h2>ACTUALIZAR INGENIERIA EN DISEÑO INDUSTRIAL</h2>
