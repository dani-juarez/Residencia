...
<?php
	...
	if(isset($_GET['id'])){
		...
		$buscar_id=$con->prepare('SELECT * FROM eventos_local2 WHERE id=:id LIMIT 1');
	...
	if(isset($_POST['guardar'])){
		...
		if(!empty($local) && !empty($hombres) && !empty($mujeres) && !empty($total) ){

				$consulta_update=$con->prepare(' UPDATE eventos_local2 SET  
					local=:local,
					hombres=:hombres,
					mujeres=:mujeres,
					total=:total
...
<body>
	<div class="contenedor">
		<h2>ACTUALIZAR EVENTO ACDEMICO LOCAL</h2>
		<form action="" method="post">
			<div class="form-group">
				<input type="text" name="local" placeholder="LOCAL" value="<?php if($resultado) echo $resultado['local']; ?>" class="input__text">
