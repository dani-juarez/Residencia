...
<?php
	...
	if(isset($_GET['id'])){
		...
		$buscar_id=$con->prepare('SELECT * FROM eventos_nacional2 WHERE id=:id LIMIT 1');
	...
	if(isset($_POST['guardar'])){
		...
		if(!empty($nacional) && !empty($hombres) && !empty($mujeres) && !empty($total) ){
				...
				$consulta_update=$con->prepare(' UPDATE eventos_nacional2 SET  
					nacional=:nacional,
					hombres=:hombres,
					mujeres=:mujeres,
					total=:total
...
<body>
	<div class="contenedor">
		<h2>ACTUALIZAR EVENTO ACDEMICO NACIONAL</h2>
		<form action="" method="post">
			<div class="form-group">
				<input type="text" name="nacional" placeholder="EVENTO NACIONAL" value="<?php if($resultado) echo $resultado['nacional']; ?>" class="input__text">
