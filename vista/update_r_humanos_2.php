...
<?php
	...
	if(isset($_GET['id'])){
		...
		$buscar_id=$con->prepare('SELECT * FROM r_humanos_2 WHERE id=:id LIMIT 1');
	...
	if(isset($_POST['guardar'])){
		...
		if(!empty($prof_medio_tiempo) && !empty($licenciatura) && !empty($especialidad) && !empty($maestria) && !empty($maestria_s_a) && !empty($doctorado) && !empty($doctorado_s_a) ){
				...
				$consulta_update=$con->prepare(' UPDATE r_humanos_2 SET  
...
<body>
	<div class="contenedor">
		<h2>ACTUALIZAR PROFESOR DE MEDIO TIEMPO</h2>
		<form action="" method="post">
			<div class="form-group">
				<input type="text" name="prof_medio_tiempo" placeholder="PROFESOR DE MEDIO TIEMPO" value="<?php if($resultado) echo $resultado['prof_medio_tiempo']; ?>" class="input__text">
