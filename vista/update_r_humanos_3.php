...
<?php
	...
	if(isset($_GET['id'])){
		...
		$buscar_id=$con->prepare('SELECT * FROM r_humanos_3 WHERE id=:id LIMIT 1');
	...
	if(isset($_POST['guardar'])){
		...
		if(!empty($prof_hora_tiempo) && !empty($licenciatura) && !empty($especialidad) && !empty($maestria) && !empty($maestria_s_a) && !empty($doctorado) && !empty($doctorado_s_a) ){
				...
				$consulta_update=$con->prepare(' UPDATE r_humanos_3 SET  
...
<body>
	<div class="contenedor">
		<h2>ACTUALIZAR PROFESOR DE HORAS ASIGNADAS</h2>
		<form action="" method="post">
			<div class="form-group">
				<input type="text" name="prof_hora_tiempo" placeholder="PROFESOR DE HORAS ASIGNADAS" value="<?php if($resultado) echo $resultado['prof_hora_tiempo']; ?>" class="input__text">
