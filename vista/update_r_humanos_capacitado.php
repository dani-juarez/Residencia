...
<?php
	...
	if(isset($_GET['id'])){
		...
		$buscar_id=$con->prepare('SELECT * FROM r_humanos_personal_cap WHERE id=:id LIMIT 1');
	...
	if(isset($_POST['guardar'])){
		...
		if(!empty($capacitado) && !empty($hombres) && !empty($mujeres) ){
				...
				$consulta_update=$con->prepare(' UPDATE r_humanos_personal_cap SET  
...
<body>
	<div class="contenedor">
		<h2>ACTUALIZAR PERSONAL CAPACITADO</h2>
		<form action="" method="post">
			<div class="form-group">
				<input type="text" name="capacitado" placeholder="PERSONAL CAPACITADO" value="<?php if($resultado) echo $resultado['capacitado']; ?>" class="input__text">
			</div>
			<div class="form-group">
