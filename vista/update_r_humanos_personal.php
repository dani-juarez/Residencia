...
<?php
	...
	if(isset($_GET['id'])){
		...
		$buscar_id=$con->prepare('SELECT * FROM r_humanos_personal WHERE id=:id LIMIT 1');
	...
	if(isset($_POST['guardar'])){
		...
		if(!empty($personal) && !empty($hombres) && !empty($mujeres) ){
				...
				$consulta_update=$con->prepare(' UPDATE r_humanos_personal SET  
					personal=:personal,
					hombres=:hombres,
					mujeres=:mujeres
					WHERE id=:id;'
	...
	<div class="contenedor">
		...
		<form action="" method="post">
			<div class="form-group">
				<input type="text" name="personal" placeholder="PERSONAL" value="<?php if($resultado) echo $resultado['personal']; ?>" class="input__text">
			</div>
			<div class="form-group">
