...
<?php
	...
	if(isset($_GET['id'])){
		...
		$buscar_id=$con->prepare('SELECT * FROM residencia_profesional_empresas2 WHERE id=:id LIMIT 1');
	...
	if(isset($_POST['guardar'])){
		...
		if(!empty($empresas) && !empty($nombre_proyecto) && !empty($creacion) && !empty($hombres_total) && !empty($mujeres_total) && !empty($total) ){
				...
				$consulta_update=$con->prepare(' UPDATE residencia_profesional_empresas2 SET  
					empresas=:empresas,
