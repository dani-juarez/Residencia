...
<?php
	...
	if(isset($_GET['id'])){
		...
		$buscar_id=$con->prepare('SELECT * FROM servicio_social_empresas2 WHERE id=:id LIMIT 1');
	...
	if(isset($_POST['guardar'])){
		...
		if(!empty($empresas) && !empty($hombres) && !empty($mujeres) && !empty($hombres_total) && !empty($mujeres_total) && !empty($total) ){
				...
				$consulta_update=$con->prepare(' UPDATE servicio_social_empresas2 SET  
					empresas=:empresas,
					hombres=:hombres,
					mujeres=:mujeres,
...
<body>
	<div class="contenedor">
		<h2>ACTUALIZAR SERVICIO SOCIAL EMPRESAS</h2>
